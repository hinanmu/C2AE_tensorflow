...
class Model(object):
    def __init__(self, config):
        ...
        self.accuracy = self.net.accuracy(self.pred, self.y)
        self.summarizer.scalar("accuracy", self.accuracy)
        self.summarizer.scalar("loss", self.loss)
        self.train = self.net.train_step(self.loss, self.learning_rate)
    ...
    def run_epoch(self, sess, data_type, summarizer, epoch, lr):
        err = list()
        i = 0
        step = epoch
        ...
        for X, Y, tot in self.data.next_batch(data_type):
            ...
            if not self.config.load:
                summ, _, pred, loss = sess.run([merged_summary, self.train, self.pred, self.loss], feed_dict=feed_dict)
    ...
    def run_eval(self, sess, data_type, summary_writer=None, step=0):
        ...
        for X, Y, tot in self.data.next_batch(data_type):
            ...
            if i == tot-1 and summary_writer is not None:
                ...
                if data_type == "validation":
                    loss_, Y_pred=  sess.run([self.loss, self.pred], feed_dict=feed_dict)

                    p_k = patk(predictions=Y_pred, labels=Y)
                else :
                    loss_, Y_pred, accuracy_val = sess.run([self.loss, self.pred, self.accuracy], feed_dict=feed_dict)
                    metrics = evaluate(predictions=Y_pred, labels=Y)
                    accuracy += accuracy_val #metrics['accuracy']
            loss += loss_
            i += 1
        X, Y = self.data.get_test()
        p_k = patk(sess.run(self.pred, feed_dict={self.x: X, self.y: Y, self.keep_prob: 1}), Y) # sess.run(self.patk, feed_dict={self.x: X, self.y: Y, self.keep_prob: 1}) #
    ...
    def fit(self, sess, summarizer):
        ...
        while self.epoch_count < max_epochs :
            ...
            loss_prediction = self.net.loss_prediction(self.y, self.keep_prob)
            output_loss = self.net.output_loss(loss_prediction, self.y)
            em_loss = self.net.embedding_loss(self.net.Fx(self.x, self.keep_prob), self.net.Fe(self.y, self.keep_prob))
            output_loss = sess.run(output_loss, feed_dict=feed_dict)
            em_loss = sess.run(em_loss, feed_dict=feed_dict)
            print('validation output_loss', output_loss / X.shape[0])
            print('validation em_loss', em_loss)
            ...
            average_loss, tr_step = self.run_epoch(sess, "train", summarizer['train'], self.epoch_count, learning_rate_epoch)
            ...
            if not self.config.debug :
                if self.epoch_count % self.config.epoch_freq == 0 :
                    ...
                    output =  "=> epochs{}/{},  Training : Loss = {:.2f} | Validation : Loss = {:.2f} | Test : Loss = {:.2f}".format(self.epoch_count, max_epochs, average_loss, val_loss, test_loss)
                    ...
                    with open("{}validation.log".format(self.config.project_prefix_path), "a+") as f:
                        ...
                        output_ += "\n=> Test : Hamming Loss = {}, Macro Precision = {}, Macro Recall = {}, Macro F Score = {}\n=> P@K = {}\n\n".format(metrics['hamming_loss'], metrics['macro_precision'], metrics['macro_recall'], metrics['macro_f1'], patk)
                        f.write(output_)
                    print(output)
...
def main():
    ...
    output += "\n=>Test :Micro F Score = {}".format(metrics['micro_f1'])
    output += "\n=>Test :Macro F Score = {}".format(metrics['macro_f1'])
